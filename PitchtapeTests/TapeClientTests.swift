...
private struct ProbeDTO: Decodable, Sendable {
    var status: Int?
    var span: TapeLooseFigure?
}

private actor ScriptedCarrier: TapeCarrying {
...
final class TapeClientTests: XCTestCase {
    private let url = URL(string: "https://zalupik-pupiuk.pro/probe")!

    func test_setsUserAgentOnEveryRequest() async throws {
        let carrier = ScriptedCarrier(results: [
            .success((Data("{\"span\":1}".utf8), http(200))),
        ])
        let client = TapeClient(carrier: carrier, holdNanoseconds: 0)
        _ = try await client.pull(ProbeDTO.self, at: url)
        let request = await carrier.recordedRequests().first
        XCTAssertEqual(request?.value(forHTTPHeaderField: "User-Agent"), TapeClient.userAgent)
        XCTAssertEqual(request?.timeoutInterval, 15)
        XCTAssertEqual(TapeClient.userAgent, "Pitchtape/1.0 (iOS; +https://zalupik-pupiuk.pro)")
    }
    ...
    func test_doesNotRetry404() async {
        ...
        do {
            ...
            XCTAssertEqual(error as? TapeWire, .vacant)
        }
        let count = await carrier.recordedRequests().count
    ...
    func test_malformedJSONIsBent() async {
        ...
        do {
            ...
            XCTAssertEqual(error as? TapeWire, .bent)
        }
    }
    ...
    func test_statusZeroIsVacant() throws {
        XCTAssertThrowsError(try TapeCatalog.accept(status: 0)) { error in
            XCTAssertEqual(error as? TapeWire, .vacant)
        }
        XCTAssertNoThrow(try TapeCatalog.accept(status: 1))
    }
    ...
    func test_lookupCancelsPriorQuery() async throws {
        ...
        let client = TapeClient(carrier: carrier, holdNanoseconds: 80_000_000)
        let endpoint = url
        ...
        do {
            ...
            XCTAssertEqual(error as? TapeWire, .waved)
            firstFailed = true
        }
